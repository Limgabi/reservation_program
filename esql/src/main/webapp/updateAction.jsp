	...
	<%
		...
		if (!userID.equals(review.getUserID())) {
			...
			if (request.getParameter("reviewTitle") == null || request.getParameter("reviewContent") == null ||
					request.getParameter("reviewTitle").equals("") || request.getParameter("reviewContent").equals("")) {
				PrintWriter script = response.getWriter();
				script.println("<script>");
			...
			int result = reviewDAO.update(reviewNum, request.getParameter("reviewTitle"), request.getParameter("reviewContent"));
			if(result == -1){
				PrintWriter script = response.getWriter();
				script.println("<script>");
